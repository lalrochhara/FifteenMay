...
from FifteenMay.modules import ALL_MODULES
from FifteenMay.modules.helper_funcs.chat_status import is_user_admin
from FifteenMay.modules.helper_funcs.decorators import FifteenMay, FifteenMay, FifteenMay
from FifteenMay.modules.helper_funcs.misc import paginate_modules
...
for module_name in ALL_MODULES:
    imported_module = importlib.import_module("FifteenMay.modules." + module_name)
...
def send_help(chat_id, text, keyboard=None):
    ...
    if not keyboard:
        ...
        # kb.append([InlineKeyboardButton(text='Support', url='https://t.me/FamhawiteIn fosys'),
...
@FifteenMay(pattern=r'start_back')
@FifteenMay(command='start', pass_args=True)
                    ...
                    [
                        [
                            InlineKeyboardButton(
                                ...
                                url='https://t.me/FifteenMaySupport',
                            ),
                            InlineKeyboardButton(
                                text=gs(chat.id, "updates_channel_link_btn"),
                                url="https://t.me/FifteenMayUpdates",
                            ),
                            InlineKeyboardButton(
                                text=gs(chat.id, "src_btn"),
                                url="https://github.com/lalrochhara/FifteenMay/",
                            ),
                        ],
                        [
                            InlineKeyboardButton(
            ...
            if args[0].lower() == "help":
                ...
                help_buttons.append(
                    [InlineKeyboardButton(text="Back", callback_data="help_back"),
                     InlineKeyboardButton(text='Support', url='https://t.me/FifteenMaySupport')]
                )
                send_help(
                    chat.id,
                    text,
...
@FifteenMay(Filters.status_update.migrate)
...
def main():
    ...
    if WEBHOOK:
        ...
        updater.start_webhook(listen="0.0.0.0", port=PORT, url_path=TOKEN, allowed_updates=Update.ALL_TYPES, 
                            webhook_url=URL+TOKEN, drop_pending_updates=FifteenMayInit.DROP_UPDATES, 
                            cert=CERT_PATH if CERT_PATH else None)
        log.info(f"FifteenMay started, Using webhooks. | BOT: [@{dispatcher.bot.username}]")

    else:
        log.info(f"FifteenMay started, Using long polling. | BOT: [@{dispatcher.bot.username}]")
        FifteenMayINIT.bot_id = dispatcher.bot.id
        FifteenMayINIT.bot_username = dispatcher.bot.username
        FifteenMayINIT.bot_name = dispatcher.bot.first_name
        updater.start_polling(timeout=15, read_latency=4, allowed_updates=Update.ALL_TYPES,
                              drop_pending_updates=FifteenMayInit.DROP_UPDATES)
    telethn.run_until_disconnected()
    updater.idle()


if __name__ == "__main__":
    log.info("[FifteenMay] Successfully loaded modules: " + str(ALL_MODULES))
