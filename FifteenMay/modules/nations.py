...
from FifteenMay.modules.helper_funcs.chat_status import whitelist_plus, dev_plus, sudo_plus
from FifteenMay.modules.helper_funcs.extraction import extract_user
from FifteenMay.modules.log_channel import gloggable
from FifteenMay.modules.sql import nation_sql as sql
from FifteenMay.modules.helper_funcs.decorators import FifteenMay
...
def addsudo(update: Update, context: CallbackContext) -> str:
    ...
    if user_id in SUPPORT_USERS:
        rt += "Requested Famhawite Infosys to promote a Support user to Sudo."
        SUPPORT_USERS.remove(user_id)

    if user_id in WHITELIST_USERS:
        rt += "Requested Famhawite Infosys to promote a Whitelist user to Sudo."
...
def addsupport(
    ...
    if user_id in SUDO_USERS:
        rt += "Requested Famhawite Infosys to demote this Sudo to Support"
    ...
    if user_id in WHITELIST_USERS:
        rt += "Requested Famhawite Infosys to promote this Whitelist user to Support"
        WHITELIST_USERS.remove(user_id)
...
@FifteenMay(command='addwhitelist')
@sudo_plus
@gloggable
...
@FifteenMay(command='addsardegna')
@sudo_plus
@gloggable
...
def removesudo(update: Update, context: CallbackContext) -> str:
    ...
    if user_id in SUDO_USERS:
        message.reply_text("Requested Famhawite Infosys to demote this user to Civilian")
        SUDO_USERS.remove(user_id)
...
@FifteenMay(command='removesupport')
@sudo_plus
@gloggable
...
@FifteenMay(command='removewhitelist')
@sudo_plus
@gloggable
...
@FifteenMay(command='removesardegna')
@sudo_plus
@gloggable
...
nations = """ FifteenMay has bot access levels we call as *"Nation Levels"*
\n*Famhawite Infosys* - Devs who can access the bots server and can execute, edit, modify bot code. Can also manage other Nations
\n*God* - Only one exists, bot owner.
Owner has complete bot access, including bot adminship in chats FifteenMay is at.
\n*Royals* - Have super user access, can gban, manage Nations lower than them and are admins in FifteenMay.
\n*Sakuras* - Have access go globally ban users across FifteenMay.
...
\n*Disclaimer*: The Nation levels in FifteenMay are there for troubleshooting, support, banning potential scammers.
Report abuse or ask us more on these at [Famhawite Infosys](https://t.me/YorktownEagleUnion).
"""
...
@FifteenMay(command=["supportlist", "sakuras"])
@whitelist_plus
...
@FifteenMay(command=["sudolist", "royals"])
@whitelist_plus
...
@FifteenMay(command=["devlist", "eagle"])
@whitelist_plus
def devlist(update: Update, context: CallbackContext):
    bot = context.bot
    true_dev = list(set(DEV_USERS) - {OWNER_ID})
    reply = "<b>Famhawite Infosys Members :</b>\n"
...
from FifteenMay.modules.language import gs
