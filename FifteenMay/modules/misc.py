...
from FifteenMay.__main__ import STATS, USER_INFO, TOKEN
from FifteenMay.modules.sql import SESSION
from FifteenMay.modules.helper_funcs.chat_status import user_admin, sudo_plus
from FifteenMay.modules.helper_funcs.extraction import extract_user
import FifteenMay.modules.sql.users_sql as sql
from FifteenMay.modules.users import __user_info__ as chat_count
...
@FifteenMay(command='id', pass_args=True)
...
@FifteenMay(command='info', pass_args=True)
...
def get_user_info(chat: Chat, user: User) -> str:
    ...
    if user.id == OWNER_ID:
        ...
        text += '\nThis Person is a part of Famhawite Infosys'
        Nation_level_present = True
...
@FifteenMay(command='echo', pass_args=True, filters=Filters.chat_type.groups)
@user_admin
...
@FifteenMay(command='markdownhelp', filters=Filters.chat_type.private)
...
stats_str = '''
'''
@FifteenMay(command='stats', can_disable=False)
@sudo_plus
def stats(update, context):
    ...
    try:
        update.effective_message.reply_text(status +
            ...
            "\n\n[⍙ GitHub](https://github.com/lalrochhara/FifteenMay) | [⍚ GitLab](https://gitlab.com/lalrochhara/FifteenMay)\n\n" +
            "╘══「 by [lalrochhara](github.com/lalrochhara) 」\n",
