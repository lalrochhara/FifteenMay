from FifteenMay.modules.helper_funcs.telethn import HIGHER_AUTH, telethn
from FifteenMay import SUPPORT_USERS, SARDEGNA_USERS, WHITELIST_USERS
...
async def FifteenMay_is_admin(chat_id: int):
    status = False
    FifteenMay = await telethn.get_me()
    async for user in telethn.iter_participants(
            chat_id, filter=ChannelParticipantsAdmins):
        if FifteenMay.id == user.id:
            status = True
            break
    return status
